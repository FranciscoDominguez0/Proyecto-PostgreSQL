# FRONTEND — ShopAI  |  Clean Dark Interface
import reflex as rx
import httpx
...
GRAD    = "linear-gradient(135deg, #22d3ee 0%, #0ea5e9 100%)"
...
class State(rx.State):
    ...
    def set_sugerida(self, p: str):   self.pregunta = p; self.pregunta_activa = p
    def handle_enter(self, key: str):
        if key == "Enter": return State.enviar()

    def limpiar(self):
        self.pregunta = self.pregunta_mostrada = self.respuesta_ia = ""
        self.sql_ejecutado = ""; self.datos_db = []
        self.error = ""; self.pregunta_activa = ""
...
def chip(t: str) -> rx.Component:
    return rx.button(
        t,
        on_click=State.set_sugerida(t),
        font_size="0.67rem", font_family=MONO,
        padding="0.22rem 0.6rem",
        border_radius="999px",
        background=rx.cond(State.pregunta_activa == t, ACCENT, DIM),
        color=rx.cond(State.pregunta_activa == t, BG, MUTED),
...
def sidebar() -> rx.Component:
    return rx.box(
        rx.text("ShopAI", font_size="1rem", font_weight="700",
                color=TEXT, font_family=MONO,
                margin_bottom="0.2rem"),
        rx.text("commerce · intelligence", font_size="0.6rem",
                color=MUTED, font_family=MONO, margin_bottom="1.5rem"),

        rx.box(height="1px", background=BORDER, margin_y="0"),
        rx.vstack(
            rx.text("DATABASE", font_size="0.57rem", color=MUTED,
                    letter_spacing="0.12em", font_family=MONO,
                    font_weight="600", margin_top="1.25rem"),
        ...
        rx.box(height="1px", background=BORDER, margin_y="1.25rem"),
        rx.vstack(
            rx.text("QUERIES", font_size="0.57rem", color=MUTED,
                    letter_spacing="0.12em", font_family=MONO, font_weight="600"),
            rx.flex(*[chip(s) for s in CHIPS], flex_wrap="wrap", gap="0.3rem",
                    margin_top="0.5rem"),
            align_items="flex-start", spacing="0", width="100%",
        ),

        rx.spacer(),
        rx.text("FastAPI · Reflex · DeepSeek",
                font_size="0.58rem", color=DIM, font_family=MONO),
    ...
    return rx.box(
        ...
        rx.box(
            ...
            rx.cond(
                ...
                rx.box(
                    rx.text("Haz una pregunta",
                            font_size="0.9rem", font_weight="500", color=MUTED),
                    rx.text("Usa los chips de la izquierda o escribe tu consulta.",
                            font_size="0.72rem", color=DIM, margin_top="0.3rem"),
                    text_align="center",
                    position="absolute",
                    top="50%", left="50%",
            ...
            rx.cond(
                ...
                rx.box(
                    rx.box(
                        rx.text(State.pregunta_mostrada, font_size="0.86rem",
                                color="white", line_height="1.6"),
                        background=GRAD,
                        padding="0.7rem 1rem",
                        border_radius="16px 16px 4px 16px",
                        max_width="72%",
                        box_shadow=f"0 2px 16px #22d3ee28",
                    ),
            ...
            rx.cond(
                ...
                rx.hstack(
                    ...
                    rx.text("procesando...", font_size="0.78rem",
            ...
            rx.cond(
                State.respuesta_ia != "",
                rx.vstack(
                    # Texto IA con markdown
                    rx.box(
                        rx.markdown(
                            State.respuesta_ia,
                            component_map={
                                "p": lambda t: rx.text(t, font_size="0.86rem",
                                                       color=TEXT, line_height="1.75",
                                                       margin_bottom="0.4rem"),
                                "strong": lambda t: rx.text(t, font_weight="700",
                                                            color=ACCENT, display="inline"),
                                "li": lambda t: rx.text("· ", t, font_size="0.86rem",
                                                        color=TEXT, line_height="1.6"),
                                "code": lambda t: rx.code(
                                    t, font_size="0.76rem", color=CODE,
                        ...
                        rx.box(
                            rx.hstack(
                                ...
                                rx.text("sql", font_size="0.6rem", color=MUTED,
                                        font_family=MONO, letter_spacing="0.12em"),
                                spacing="2", align="center", margin_bottom="0.5rem",
                            ),
                            rx.text(State.sql_ejecutado,
            ...
            rx.hstack(
                rx.input(
                    ...
                    _focus={"border_color": ACCENT,
                            "box_shadow": f"0 0 0 2px #22d3ee22",
                            "outline": "none"},
                    transition="all 0.2s",
                ),
                rx.button(
                    ...
                    background=GRAD, color=BG,
                    font_family=MONO, font_size="0.78rem",
                    font_weight="700", border="none",
                    border_radius="10px",
