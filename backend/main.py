...
@app.get("/")
def root():
    return {"mensaje": "ShopAI API activa", "version": "1.0.0"}
...
@app.get("/stats")
def stats():
    """Estadisticas rapidas del sistema."""
    try:
        clientes  = ejecutar_query("SELECT COUNT(*) as total FROM clientes")
        productos = ejecutar_query("SELECT COUNT(*) as total FROM productos")
        pedidos   = ejecutar_query("SELECT COUNT(*) as total FROM pedidos")
        ventas    = ejecutar_query(
            "SELECT COALESCE(SUM(total), 0) as total FROM pedidos WHERE estado='entregado'"
        )
        return {
            "clientes":     clientes[0]["total"],
            "productos":    productos[0]["total"],
            "pedidos":      pedidos[0]["total"],
            "ventas_total": float(ventas[0]["total"]),
        }
    except Exception as e:
